...
class BaseComputerTool:
    """
    A tool that allows the agent to interact with the screen, keyboard, \
    and mouse of the current computer.
        ...
        if action in ("mouse_move", "left_click_drag"):
            ...
            if action == "left_click_drag":
                ...
                command_parts = [
                    ...
                    mousedown 1 mousemove --sync {end_x} {end_y} mouseup 1",
                ]
    ...
    async def __call__(
        ...
        **kwargs,
        ...
        if action == "zoom":
            ...
            width = x1 - x0
            height = y1 - y0
            crop_cmd = f"convert {temp_path} -crop {width}x{height}+{x0}+{y0} \
            +repage {cropped_path}"
            await run(crop_cmd)
